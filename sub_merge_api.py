...
def create_caption(textJSON, framesize, v_type, font="Arial", color='white', highlight_color='orange', stroke_color='black', stroke_width=1.5):
    ...
    if v_type.lower() != 'reels':
        fontsize = int(frame_height * 0.095)  # 7.5 percent of video height
    else:
        fontsize = int(frame_height * 0.09)  # 5 percent of video height for Reels
    ...
    for index, wordJSON in enumerate(textJSON['textcontents']):
        ...
        word_clip = TextClip(wordJSON['word'], font

=font, fontsize=fontsize, color=color, stroke_color=stroke_color, stroke_width=stroke_width).set_start(textJSON['start']).set_duration(full_duration)
...
def get_final_clipped_video(videofilename, linelevel_subtitles, v_type):
...
def add_subtitle(videofilename, audiofilename, v_type):
    ...
    outputfile = get_final_clipped_video(videofilename, linelevel_subtitles, v_type)
    return outputfile
...
def get_processed_video():
    if outputfile:
        #return jsonify({'outputfile_path': outputfile}), 200
        
        return send_file(outputfile, as_attachment=True), 200
    else:
        return jsonify({'error': 'Processed video file path not available'}), 404
...
def process_video():
    ...
    try:
        ...
        videofilename = os.path.join(directory, 'input_video.mp4')
        file.save(videofilename)

        # Call your existing functions to process the video
        # For example:
        # audiofilename = create_audio(videofilename)
        # outputfile = add_subtitle(videofilename, audiofilename, v_type)

        audiofilename = create_audio(videofilename)
        outputfile = add_subtitle(videofilename, audiofilename, "Video")

        # Here you would call your existing functions to process the video
        # For demonstration purposes, let's assume the outputfile is the processed video file

        #outputfile = videofilename  # Placeholder for processed video file

        # Check if the outputfile exists
        if os.path.exists(outputfile):
            return send_file('C:\\Users\\HP\\AppData\\Local\\Temp\\output.mp4', as_attachment=True), 200
            # Send the processed video file as a response
            #return jsonify({'outputfile_path': outputfile}), 200
            
            #return jsonify({'success': 'File processed successfully'}), 200
        else:
            return jsonify({'error': 'Error processing the video'}), 500
